...
class GestureRecognizer:
    ...
    def compute_circularity(self, img):
    ...
    def compute_convex_solidity(self, img):
        contours, _ = cv2.findContours(img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if not contours:
            return 0.0
        largest_contour = max(contours, key=cv2.contourArea)
    ...
    def compute_stroke_complexity(self, image):
        if len(image.shape) == 3:
            image = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
        ...
        if not contours:
            return 0
        largest_contour = max(contours, key=cv2.contourArea)
        path_length = sum(np.linalg.norm(largest_contour[i] - largest_contour[i-1]) for i in range(1, len(largest_contour)))
        x, y, w, h = cv2.boundingRect(largest_contour)
        bounding_perimeter = 2 * (w + h)
        if bounding_perimeter == 0:
            return 0
        stroke_complexity_ratio = path_length / bounding_perimeter
        return stroke_complexity_ratio
    ...
    def get_language_features(self):
        ...
        for gesture, info in self.language.items():
            ...
            _, img = cv2.threshold(img, 0.1, 255, cv2.THRESH_BINARY)
            features[gesture] = {
                "gesture": gesture,
                ...
                "circularity": self.compute_circularity(img),
                "solidity": self.compute_convex_solidity(img),
                "stroke_complexity": self.compute_stroke_complexity(img),
            }
        return features
    ...
    def identify_gesture(self, current_keypoints):
        ...
        fd_diffs = []
        canny_diffs = []
        circularity_diffs, solidity_diffs, aspect_ratio_diffs, stroke_diffs, edge_diffs = [], [], [], [], []
        for gesture, features in self.language_features.items():
            ...
            _, image_array = cv2.threshold(image_array, 0.1, 255, cv2.THRESH_BINARY)
            ...
            current_circularity = self.compute_circularity(image_array)
            current_solidity = self.compute_convex_solidity(image_array)
            current_aspect_ratio = self.compute_aspect_ratio(image_array)
            current_stroke_complexity = self.compute_stroke_complexity(image_array)

            circularity_diffs.append(abs(current_circularity - features['circularity']))
            solidity_diffs.append(abs(current_solidity - features['solidity']))
            aspect_ratio_diffs.append(abs(current_aspect_ratio - features['aspect_ratio']))
            stroke_diffs.append(abs(current_stroke_complexity - features['stroke_complexity']))
        ...
        circularity_vote = np.argmin(circularity_diffs)
        solidity_vote = np.argmin(solidity_diffs)
        aspect_ratio_vote = np.argmin(aspect_ratio_diffs)
        stroke_complexity_vote = np.argmin(stroke_diffs)

        hu_conf = 1 / (hu_diffs[hu_vote] + 1e-7)
        zernike_conf = 1 / (zernike_diffs[zernike_vote] + 1e-7)
        fd_conf = 1 / (fd_diffs[fd_vote] + 1e-7)
        votes = [hu_vote, zernike_vote, fd_vote, circularity_vote, solidity_vote, aspect_ratio_vote, stroke_complexity_vote]
        print(f"PRIMARY VOTERS:")
        ...
        print(f"SECONDARY VOTERS:")
        print(f"circularity: {circularity_vote}")
        print(f"solidity: {solidity_vote}")
        print(f"aspect_ratio: {aspect_ratio_vote}")
        print(f"stroke_complexity: {stroke_complexity_vote}")
        vote_counts = {}
        for vote in votes:
